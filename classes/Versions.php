			...
			if ($intTo > 0 && $intFrom > 0)
			{
				...
				foreach ($to as $k=>$v)
				{
					if ($from[$k] != $to[$k])
					{
						...
						{
							$to[$k] = class_exists('Contao\StringUtil') ? \StringUtil::binToUuid($to[$k]) : \StringUtil::binToUuid($to[$k]);
						}
						if ($blnIsBinary && \Validator::isBinaryUuid($from[$k]))
						{
							$to[$k] = class_exists('Contao\StringUtil') ? \StringUtil::binToUuid($from[$k]) : \StringUtil::binToUuid($from[$k]);
						}
...
';
	...
	public static function addToTemplate(\BackendTemplate $objTemplate)
	{
		...
		while ($objVersions->next())
		{
			...
			$arrRow['description'] = class_exists('Contao\StringUtil') ? \StringUtil::substr($arrRow['description'], 32) : \StringUtil::substr($arrRow['description'], 32);
			$arrRow['shortTable'] = class_exists('Contao\StringUtil') ? \StringUtil::substr($arrRow['fromTable'], 18) : \StringUtil::substr($arrRow['fromTable'], 18); // see #5769
	...
	protected function implodeRecursive($var, $binary=false)
	{
		if (!is_array($var))
		{
			return $binary ? (class_exists('Contao\StringUtil') ? \StringUtil::binToUuid($var) : \StringUtil::binToUuid($var)) : $var;
		}
		elseif (!is_array(current($var)))
		{
			if ($binary)
			{
				$var = array_map(function ($v) { return $v ? (class_exists('Contao\StringUtil') ? \StringUtil::binToUuid($v) : \StringUtil::binToUuid($v)) : ''; }, $var);
			}
